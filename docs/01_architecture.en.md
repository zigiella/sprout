...
Pollen talks to Meristem either over WebSocket or a parallel HTTP fallback (added after a real field test on day 27). Meristem listens on port 13000. **In the MVP the Meristem and Rhizome base URLs are hardcoded in the Pollen Android client** (`VisitarMeristemScreen.kt` and `VisitarRhizomeScreen.kt`); a Settings screen for runtime URL configuration and mDNS / NSD auto-discovery are roadmap for v1.1 (see §9 Out of scope). **Nothing leaves the home network** — no cloud, no accounts, no telemetry.
...
- **Runtime configuration of Meristem and Rhizome URLs in the Pollen Android client.** The MVP hardcodes the base URLs in `VisitarMeristemScreen.kt` and `VisitarRhizomeScreen.kt`. To point the app at a different Meristem or Rhizome instance currently requires editing those two lines and rebuilding the APK. A Settings screen (two URL fields persisted via `PollenStore`) and mDNS / NSD auto-discovery are roadmap for v1.1.

...
- `bitacora/2026-04-30_endodermis-onboarding-repo-vivo_endodermis.md`
- `coordinacion/2026-04-30_reu_tripartita_endo_xilema/bloque_1_temas_a_tratar/temas_endodermis.md`
- `coordinacion/2026-04-30_reu_tripartita_endo_xilema/bloque_1_temas_a_tratar/enunciado_bloque_1.md`

El onboarding de Endodermis ya esta en `main`; mi primera respuesta quedo escrita antes de ese fast-forward, asi que este archivo queda ajustado tras leerlo.
...
7. **Adapter `llamacpp`.** Tras leer a Endodermis, acepto su propuesta: usar `code/meristem_inference_adapter` como puente oficial para la bateria Rhizome contra `llama-server`, no crear un runner nuevo salvo fallo real del adapter.
...
3. **Comando exacto de bateria Rhizome v0.5.** `code/tuning/` ya esta en main. Necesito que Endodermis identifique el comando minimo reproducible para ejecutar v0.5 contra endpoint llama.cpp y que lo deje escrito antes de correr la bateria completa. Su propuesta de `ADAPTER_PORT=12000` con `LLAMACPP_SERVER_URL=http://localhost:8080` me parece el punto de partida correcto.

4. **Artefactos de resultados.** Acepto la propuesta de Endodermis: JSONL completos en `code/tuning/results/` gitignored y resumen versionado en bitacora. Si hay un reporte pequeno que merezca versionarse, se decide caso por caso.
...
6. **Cuantizacion.** Confirmo mi preferencia: primera medicion oficial con `Q4_K_S`, porque es lo documentado en `docs/51`/`docs/52` y deja margen en 8 GB. `Q4_K_M` queda como comparativa posterior si E2B `Q4_K_S` pasa.
...
5. **Criterio de exito.** Para MVP buscamos `engineering pass`, no production-ready: repetible en frio/caliente, sin OOM, sin truncamiento critico y con resultados explicables. Mi propuesta concreta: `engineering pass` requiere repetir los 6 criticos en frio/caliente y que los 18 no tengan fallo critico; si los 18 completos tambien se repiten sin degradacion, mejor, pero no lo pondria como minimo del dia 17.
